starch
{
Noun:An odorless testeless white substance occurring widely in plant tissue and obtained chiefly from cereals and potatoes. It is a polysaccharide that functions as a carbohydrate store and is an important constituent of the human diet.
Verb:Stiffen (fabric or clothing) with starch.
}
stark
{
Adjective:Severe or bare in appearance or outline.
    Unpleasantly or sharply clear; impossible to avoid.
}
startling
{
Adjective:Very surprising, astonishing, or remarkable.
}
stash
{
Noun:A secret store of something.
    A hiding place or hideout.
Verb:Store (something) safely and secretly in a specified place.
}
stationary
{
Adjective:Not moving or not intended to be moved.
}
stature
{
Noun:A person's natural height.
    Importance or reputation gained by ability or achievement.
}
steep
{
Adjective:(of a slope, flight of stairs, angle, ascent, etc.) Rising or falling sharply; nearly perpendicular.
Verb:Soak (food or tea)in water or other liquid so as to extract its flavor or to soften it.
}
steer
{
Noun:A piece of advice or information concerning the development of a situation.
Verb:(of a person)Guide or control the movement of (a vehicle, vessel, or aircraft), for example by turning a wheel or operating a rudder.
}
stellar
{
Adjective:Of or relating to a star or stars.
    Featuring or having the quality of a star performer or performers.
    Exceptionally good; outstanding.
}
stem
{
Verb:Stop or restrict(the flow of something).
Verb:Originate in or be caused by. Remove the stems from (fruit or tobacco leaves).
Noun:The main body or stalk of a plant or shrub, typically rising above ground but occasionally subterranean.
}
stereo
{
Noun:Sound that is directed through two or more speakers so that it seems to surround the listener and to come from than one source; stereophonic sound.
}
sterile
{
Adjective:Not able to produce children or young.
    (of a plant)Not able to produce fruit or seeds.
}
stew
{
Verb:Cook (meat, fruit, or other food)slowly in liquid in a closed dish or pan.
    (of meat, fruit, or other food)Be cooked in such a way.
Noun:A pond or large tank for keeping fish for eating.
}
stick
{
Noun:A thin piece of wood that has fallen or been out from a tree.
    A thin piece of wood that has been trimmed for a particular purpose, in particular.
}
stiff
{
Verb:Cheat(someone) out of something, esp. money.
    Fail to leave (someone) a tip.
Noun:A dead body. A boring, conventional person.
Adjective:Not easily bent or changed in shape; rigid.
}
sting
{
Noun:A small sharp-pointed organ at the end of the abdomen of bees, wasps, ants, and scorpions, capable of inflicting a painful or dangerous wound by injecting poison.
Verb:Wound or pierce with a sting.
}
stir
{
Verb:Move a spoon or other implement around in (a liquid or other substance)in order to mix it thoroughly.
    Add an ingredient to (a liquid or other substance)in such a way.
Noun:Prison.
Noun:A slight physical movement.
}
stout
{
Adejective:(of a person)Somewhat fat or of heavy build.
    (of an object)Strong and thick.
}
strain
{
Noun:A force tending to pull or stretch something to an extreme or damaging degree.
    The magnitude or a deformation, equal to the change in the dimension of a deformed object divided by its original demension.
Noun:A breed, stock, or variety of an animal or plant developed by breeding.
Verb:Force(a part of one's body or oneself)to make a strenuous or unusually great effort).
}
strand
{
Noun:A single thin length of something such as thread, fiber, or wire, esp. as twisted together with others.
Verb:Drive or leave(a boat, sailor, or sea creature)aground on a shore.
}
strap
{
Verb:Fasten or secure in a specified place or position with a strap or seat belt.
Noun:A strip of leather, cloth or other flexible material, often with a buckle, used to fasten, secure, or carry something or to hold on to something.
}
stratum
{
Noun:A layer or a series of layers of rock in the ground.
    A thin layer within any structue.
}
stride
{
Noun:A long, decisive step. The length of a step or manner of taking steps in walking or running.
Verb:Walk with long, decisive steps in a specified direction.
}
strike
{
Verb:Hit forcibly and deliberately with one's hand or a weapon or other implement.
    Accidentally hit(a part of one's body)against something.
Noun:A refusal to work organized by a body of employees as a form of protest, typically in an attempt to gain a concession or concessions from their employer.
}
string
strip
{
Verb:Remove all coverings from.
    Remove the clothes from (someone).
Noun:A long, narrow piece of cloth, paper, plastic, or some other material.
Noun:Used for or involving the performance of stripteases.
}
stripe
{
Verb:Mark with stripes.
Noun:A long narrow band or strip, typically of the same width throughout its length, differing in color or texture from the surface on either side of it.
}
strive
{
Verb:Make great efforts to achieve or obtain something.
    Struggle or fight vigorously.
}
stun
{
Verb:Knock unconscious or into a dazed or semiconscious state.
    Astonish or shock (someone) so that they are temporarily unable to react.
}
stunt
{
Verb:Retard the growth or development of.
Noun:An action displaying spectacular skill and daring.
Verb:Perform stunts, esp. aerobatics.
}
sturdy
{
Noun:Vertigo in sheep caused by a tapeworm larva encysted in the brain.
Adjective:(of a person or their body)Strongly and solidly built.
}
stylistic
{
Adjective:Of or concerning style, esp. literary style.
}
subdue
{
Verb:Overcome, quieten, or bring under control(a feeling or person).
    Bring (a country or people)under control by force.
}
submerge
{
Verb:Cause to be under water.
    Descend below the surface of an area of water.
    Completely cover or obscure.
}
subsequent
{
Adjective:Coming after something in time; following.
    (of a stream or valley)Having a direction or character determined by the resistance to erosion of the underlying rock, and typically following the strike of the strata.
}
subsidize
{
Verb:Support(an organization or activity)financially.
    Pay part of the cost of producing(something)to reduce prices for the buyer.
}
subsidy
{
Noun:A sum of money granted by the government or a public body to assist an industry or business so that the price of a commodity or service may remain low or competitive.
    A sum of money granted to support an arts organization or other undertaking held to be in the public interest.
}
subsist
{
Verb:Maintain or support oneself, esp. at a minimal level.
    Provide sustenance for.
}
substantiate
{
Verb:Provide evidence to support or prove the truth of.
}
