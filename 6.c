collaborator
{
Noun:A person who works jointly on an activity or project; an associate.
    A person who cooperates traitorously with an enemy; a defector.
}
collectively
{
Adverb:Wholely.
}
collide
{
Verb: Hit with force when moving. Come into conflict or opposition.
}
combustible
{
Adjective: Able to catch fire and burn easily.
}
commentary
{
Noun: The expression of opinions or explanations about an event or situation.
}
commission
{
Verb: Order or authorize(a person or organization)to do or produce something. 
Noun:The authority to perform a task or certain duties.
}
communal
{
Adjective: Shared by all members of a community; Of, relating to, or done by a community.
}
commute
{
Noun: A regular journey of some distance to and from one's place of work. 
Verb: Travel some distance between one's home and place of work on a regular basis.
}
compact
{
Verb: Make or enter into(a formal agreement)with another party or parties. 
Noun: A formal agreement or contract between two or more parties. 
Adjective: Closely and neatly packed together; dense.
}
compass
{
Verb: Go around(something)in a circular course. 
Noun: An instrument containing a magnetized pointer that shows the direction of magnetic north and bearings from it.
}
compel
{
Verb: Force or oblige(someone)to do something. Bring about(something)by the use of force or pressure.
}
oblige
{
Verb: Do as (someone)asks or desires in order to help or please them.
}
compensate
{
Verb: Recompense(someone)for loss, suffering, or injury, typically by the award of a sum of money.
}
competence
{
Noun: The ability to do something successfully or efficiently.
}
compile
complement
comply
{
Verb:(of a person or group)Act in accordance with a wish or command.
Verb:(of an article)Meet specified standards. 
}
comprehend
{
Verb: Grasp mentally; understand.
}
compress
{
Noun: A pad of absorbent material pressed onto part of the body to relieve inflammation or stop bleeding. 
Verb: Flatten by pressure; squeeze; press.
}
comprise
{
Verb: Consist of; be made up of. Make up; constitute.
}
compromise
{
Noun: An agreement or a settlement of a dispute that is reached by each side making concessions.
}
conceal
{
Verb: Keep (something) secret; prevent from being known or noticed.
}
concede
{
Verb: Admit that something is true or valid after first denying or resisting it.
}
conceive
{
Verb: Become pregnant with(a child). Form or devise(a plan or idea)in the mind.
}
concise
{
Adjective:Giving a lot of information clearly and in a few words; brief but comprehensive.
}
conclusive
{
Adjective:(of evidence or argument)Serving to prove a case; decisive or convincing.
}
condensation
{
Noun: The conversion of a vapor or gas to a liquid.
}
condense
{
Verb: Make(something)denser or more concentrated.
}
alliance
{
Noun: A union or association formed for mutual benefit, esp. between countries or organizations.
}
confederacy
{
Noun: An alliance formed for an unlawful purpose; a conspiracy.
}
bestow
{
Verb: Confer or present(an honor, right, or gift).
}
confer
{
Verb: Grand or bestow(a title, degree, benefit, or right).
}
confession
{
Noun: A formal statement admitting that one is guilty of a crime.
}
bully
{
Adjective:Very good; first rate.
Noun: A person who uses strength or power to harm or intimidate those who are weaker. 
Verb:Use superior strength or influence to intimidate(someone), typically to force him or her to do what one wants.
}

//unfamiliar
combustible
compensate
competence
conceive
concise
