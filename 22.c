occasional
{
Adjective:Occurring, appearing, or done infrequently and irregularly.
}
oceanographer
octopus
cactus
odd
odor
{
Noun:A distincitve smell, esp. an unpleasant or distinctive one.
}
offensive
{
Noun: An attacking military compaign. 
Adjective: Causing someone to feel deeply hurt, upset, or angry.
}
olfactory
{
Adjective: Of or relating to the sense of smell.
}
onslaught
{
Noun: A fierce or destructive attack.
}
ooze
{
Verb:(of a fluid)Slowly trickle or seep out of something; flow in a very gradual way.
}
opal
{
Noun: A gemstone consisting of hydrated silica, typically semitransparent and showing varying colors against a pale or dark ground.
}
opaque
{
Noun:An opaque thing or substance. 
Adjective:Not able to be seen through; not transparent.
}
optical
{
Adjective: Of or relating to sight, esp. in relation to the physical action of light.
}
optimal
{
Adjective: Best or most favorable; optimum;
}
orchestra
{
Noun:A group of instrumentalists, esp. one combining string, woodwind, brass, and precussion sections and playing classical music.
}
orchid
{
Noun: A plant with complex flowers that are typically showy or bizarrely shaped.
}
ore
{
Noun: A naturally occurring solid materia from which a metal or valuable mineral can be profitably extracted.
}
orientation
{
Noun:The determination of the relative position of something or someone(esp. oneself). 
    The relative physical position or direction of something.
}
outfit
{
Noun: A set of clothes worn together, typically for a particular occasion or purpose. 
Verb: Provide(someone)with a set of clothes.
}
outgas
{
Verb:Release or give off(a substance)as a gas or vapor.
}
outrageous
{
Adjective:Shocking bad or excessive.
}
overhaul
{
Noun: A thorough examination of machinery or a system. 
Verb: Take apart(a piece of machinery or equipment)in order to examine it and repair it if necessary.
}
overwhelm
{
Verb:Bury or drown beneath a huge mass. Give too much of a thing to (someone);
}
owl
{
Noun: A nocturnal bird of prey with large forward-facing eyes surrounded by facial disks, a hooked beak, and typically a loud call.
}
oxide
{
Noun: A binary compound of oxygen with another element or group.
}
paddle
{
Noun: A short pole with a broad blade at one or both ends, used without an oarlock to move a small boat or canoe through the water.
}
painstaking
{
Adjective: Done with employing great care and thoroughness.
}
palatable
{
Adjective:(of food or drink)Pleasant to taste.
}
palate
{
Noun:the roof of the mouth, separating the cavities of the nose and the mouth in vertebrates. A person's appreciation of taste and flavor, esp. when sophisticated and discriminating.
}
pale
{
Noun: A wooden stake or post used as an upright along with others to form a fence. 
Verb: Become pale in one's face from shock or fear. 
Adjective: Light in color or having little color.
}
pamphlet
{
Verb: Distribute pamphlets to. 
Noun:A small booklet or leaflet containing information or arguments about a single subject.
}
pancreas
{
Noun: A large gland behind the stomach that secretes digestive enzymes into the duodenum.
}
panel
{
Verb:Cover(a wall or other surface)with panels. 
Noun:A thin, typically rectangular piece of wood or glass forming or set into the surface of a door, wall, or ceiling.
}
pants shorts
{
Noun: Trousers, Underpants, Rubbish; nonsense.
}
paralyze
{
Verb: Cause(a person or part of the body)to become partly or wholly incapable of movement. Render(someone)unable to think or act normally, esp. through panic or fear.
}
paramount
{
Adjective: More important than anything else; supreme. Having supreme power.
}
paraphrase
{
Noun:A rewording of something written or spoken by someone else. 
Verb: Express the meaning of(the write or speaker or something written or spoken)using different words, esp. to achieve greater clarity.
}
parasite
{
Noun:An organism that lives in or on another organism(its host)and benefits by deriving nutrients at the host's expense.
}
particle
{
Noun:A minute portion of matter. The least possible amount.
}
passive
{
Adjective: Accepting or allowing what happens or what others do, without active reponse or resistance.
}
paste
{
Noun: A thick, soft, moist substance, usually produced by mixing dry ingredients with a liquid. 
Verb: Coat with paste.
}
pasture
{
Verb: Put(animals)in a pasture to graze. 
Noun: Land covered with grass and other low plants suitable for grazing animals, esp. cattle or sheep.
}
patch
{
Noun:A piece of cloth or other material used to mend or strengthen a torn or weak point.
}
patent
{
Verb:Obtain a patent for(an invention). 
Adjective: Easily recognizable; obvious. 
Noun: A government authority to an individual or organization conferring a right or title, esp. the sole right to make, use, or sell some invention.
}
//unfamiliar
onslaught
ooze
optical
ore
outfit
outrageous
overwhelm
painstaking
palatable
palate
pancreas
paralyze
paramount
paraphrase
parasite
pasture
patch
offensive
