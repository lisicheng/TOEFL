excess
{
Adjective: Exceeding a prescribed or desirable amount. 
Noun: An amount of something that is more than necessary, permitted, or desirable.
}
excursion
{
Noun: A short journey or trip, esp. one engaged in as a leisure activity.
}
leisure
exemplary
{
Adjective:Serving as a desirable model; representing the best of its kind.
}
exempt
{
Adjective:Free from an obligation or liability imposed on others.
}
obligation
{
Noun: an act or course of action to which a person is morally or legally bound; a duty or commitment.
}
liability
{
Noun: A thing for which someone is responsible, esp. a debt or financial obligation.
}
exert
{
Verb: Apply or bring to bear(a force, influence, or quality). Make a physical or mental effort.
}
exhale
{
Verb: Breathe out in a deliberate manner. Give off(vapor or fumes).
}
exhaust
{
Noun:Waste gases or air expelled from an engine, turbine, or other machine in the course of its operation. 
Verb:Drain(someone)of their physical or mental resources; tire out.
}
exhilarating
{
Adjective: Making one feel very happy, animated, or elated; thrilling.
}
exotic
{
Noun:A thing that is imported or unusual. 
Adjective: Originating in or characteristic of a distant foreign country.
}
expel
{
Verb: Deprive(someone)of membership of or involvement in a school or other organization. Force (someone)to leave a place, esp. a country.
}
expertise
{
Noun:Expert skill or knowledge in a particular field.
}
expire
{
Verb: (of a document, authorization, or agreement)Cease to be valid, typically after a fixed period of time. Come to an end. (of a person)Die.
}
exploit
{
Noun: A bold or daring feat. 
Verb: Make full use of and derive benefit from(a resource). Use(a situation or person)in an unfair or selfish way.
}
extent
{
Noun:The area covered by something. The degree to which something has spread; the size or scale of something.
}
exterior
{
Noun: The outer surface or structure of something. 
Adjective: Forming, situated on, or relating to the outerside of something. interior.
}
extinct
{
Adjective:(of a species, family, or other larger group)Having no living members. No longer in existence.
}
extol
{
Verb: Praise enthusiastically.
}
extract
{
Verb: Remove or take out, esp. by effort or force. Derive(an idea or the evidence for it)from a body of information. 
Noun: A short passage taken from a piece of writing, music, or film.
}
extraordinary
{
Noun: An item in a company's accounts not arising from its normal activities. 
Adjective:Very unusual or remarkable. Unusually great. (of a meeting)Specially convened.
}
extravagant
{
Adjective: Lacking restraint in spending money or using resources. Costing too much money.
}
evacuate
excavate
exuberant
{
Adjective: Filled with or characterized by a lively energy and excitement.
}
fabricate
{
Verb:Invent or concoct(something), typically with deceitful intent. Construct or manufacture(something, esp. ans industrial product), esp. from prepared components.
}
facade
{
Noun: The face of a building, esp. the principal front that looks onto a street or open space.
}
facilitate
{
Verb: Make (an action or process)easy or easier.
}
Fahrenheit
{
Adjective: Of or denoting a scale of temperature on which water freezes at 32 degrees and boils at 212 degrees under standard conditions. 
Noun: This scale of temperature.
}
faint
{
Verb: Lose consciousness for a short time because of a temporarily insufficient supply of oxygen to the brain. 
Noun: A sudden loss of consciousness. 
Adjective:(of a sight, smell, or sound)Barely perceptible.
}
fare
{
Noun:The money a passenger on public transportation has to pay. 
Verb:Perform in a specified way in a particular situation or over a particular period of time. Happen; turn out.
}
fascinate
{
Verb: Draw irresistibly the attention and interest of(someone).
}
fatigue
{
Noun: Extreme tiredness, typically resulting from mental or physical exertion or illness. 
Verb:Cause(someone)to feel tired or exhausted.
}
faucet
{
Noun: A device by which a flow of liquid or gas from a pipe or container can be controlled; a tap.
}
fauna
{
Noun: The animals of a particular region, habitat, or geological period.
}
feasible
{
Adjective: Possible to do easily or conveniently. Likely; probable.
}
feat
{
Noun:An achievement that requires great courage, skill, or strength.
}
feeble
{
Adjective: Lacking physical strength, esp. as a result of age or illness.
}
ferment
{
Verb: (of a substance)Undergo fermentation. Cause the fermentation of(a substance). 
Noun:A fermenting agent or enzyme.
}
fertile
feudal
{
Adjective: According to, resembling, or denoting the system of feudalism. Absurdly outdated or old-fashioned.
}

//unfamiliar
exert
exhilarating
exploit
faint
feasible
